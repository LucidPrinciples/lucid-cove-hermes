...
function _plRestoreNowPlaying() {
    ...
    if (!mount) return;
    const snap = (typeof _otActiveQueue !== 'undefined') ? _otActiveQueue : null;
    const source = (typeof _otSource !== 'undefined') ? _otSource : '';
    if (source !== 'playlist' && !(snap && snap.source === 'playlist')) return;
    if (typeof _otSyncQueueToAudio === 'function') {
        if (!_otSyncQueueToAudio()) return;
    } else if (typeof otTracks === 'undefined' || !otTracks.length) {
        return;
    }
