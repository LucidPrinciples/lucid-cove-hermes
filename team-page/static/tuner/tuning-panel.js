...
let _otActiveQueue = null;      // last otSetPlaylist snapshot — restore UI without touching audio
...
function otGetAudioUrl(t) {
    ...
    return (t.cdnBase || OT_AUDIO_BASE) + '/' + folder + '/' + filename;
}
...
function _otPaintPreview(mountId, tracks, opts) {
    ...
    root.querySelectorAll('.ot-fav-btn').forEach(el => el.setAttribute('onclick', ''));
    root.querySelectorAll('.ot-controls .ot-btn:not(.ot-play)').forEach(el => {
        el.setAttribute('onclick', '_otStartPreview(0)');
    });
    const vol = root.querySelector('.ot-volume input');
    if (vol) vol.removeAttribute('oninput');
    const toggle = root.querySelector('.ot-playlist-toggle');
    if (toggle) {
        toggle.setAttribute('onclick', '');
        toggle.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const tracksEl = root.querySelector('.ot-playlist-tracks');
            if (!tracksEl) return;
            const hide = tracksEl.style.display !== 'none';
            tracksEl.style.display = hide ? 'none' : 'block';
...
function otRenderTuning(data) {
    ...
    // Frequency — same pattern as Drop: name only, no ALIGNMENT
    const alignEl = document.getElementById('otAlignment');
    if (alignEl) {
        alignEl.hidden = false;
        alignEl.textContent = freq || '';
        alignEl.style.color = freqColor;
    }
...
function otSetPlaylist(tracks, opts) {
    ...
    const live = !!(otAudio && otAudio.src);
    // Opening Tune, Playlists, or Drop is not Play — even if the live queue is paused.
    ...
    _otActiveQueue = {
        tracks: tracks.slice(),
        label: _otPlaylistLabel,
        source: _otSource,
        freqColor: opts.freqColor || window._otFreqColor || '',
    };
...
function _otFilenameInSrc(filename, src) {
    if (!filename || !src) return false;
    try {
        if (src.indexOf(filename) !== -1) return true;
        if (src.indexOf(encodeURIComponent(filename)) !== -1) return true;
    } catch (e) {}
    return false;
}

function _otLiveTrackIndex(tracks) {
    const src = (otAudio && (otAudio.currentSrc || otAudio.src)) || '';
    if (!src || !tracks || !tracks.length) return -1;
    return tracks.findIndex(t => t && _otFilenameInSrc(t.filename, src));
}

/**
 * Point otTracks/otIndex at the file actually in otAudio.
 * Does not pause, seek, or assign .src — playback-consistency path stays intact.
 */
function _otSyncQueueToAudio() {
    const liveIdx = _otLiveTrackIndex(otTracks);
    if (liveIdx >= 0) {
        otIndex = liveIdx;
        return true;
    }
    if (_otActiveQueue && _otActiveQueue.tracks && _otActiveQueue.tracks.length) {
        const snapIdx = _otLiveTrackIndex(_otActiveQueue.tracks);
        if (snapIdx >= 0) {
            otTracks = _otActiveQueue.tracks.slice();
            otIndex = snapIdx;
            _otSource = _otActiveQueue.source || _otSource;
            _otPlaylistLabel = _otActiveQueue.label || _otPlaylistLabel;
            if (_otActiveQueue.freqColor) window._otFreqColor = _otActiveQueue.freqColor;
            _otPendingPlay = false;
            return true;
        }
    }
...
function _otBindMediaSessionHandlers() {
    ...
    // Lock screen ±10s buttons still fire seekbackward/forward on many OSes —
    // map those to skip tracks instead of jumping 10 seconds.
    try { ms.setActionHandler('seekbackward', () => otPrev()); } catch(e){}
    try { ms.setActionHandler('seekforward', () => otNext()); } catch(e){}
